...
import { MeteorEvent } from '../systems/MeteorEvent';

interface SurfaceSceneData {
  skipIntro?: boolean;
  previewX?: number;
  previewWater?: boolean;
  previewMeteorElapsedMs?: number;
}
...
export class SurfaceScene extends Phaser.Scene {
  ...
  private previewMeteorElapsedMs?: number;
  ...
  public init(data: SurfaceSceneData): void {
    ...
    this.previewMeteorElapsedMs = data.previewMeteorElapsedMs;
  }
  ...
  private startMeteorEvent(startedAtEpochMs: number): void {
    ...
    this.meteor = new MeteorEvent(this, {
      startedAtEpochMs,
      crt: this.crt,
      onLockPlayer: () => {
        if (this.state === GameState.METEOR_IMPACT) return;
        this.state = GameState.METEOR_IMPACT;
        this.player.setControlEnabled(false);
        this.player.setAcceleration(0).setVelocity(0);
      },
      onImpact: () => this.physics.pause(),
    });
    AudioSystem.instance.startMeteorRumble();
  }
